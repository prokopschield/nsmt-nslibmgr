...
exports.cloudHandler = exports._upload_dir = exports._upload_file = exports.compileHandler = exports.testHandler = exports.publishHandler = exports.creativeHandler = exports.DEFAULTS = exports.ERROR = void 0;
...
async function creativeHandler(path = '.') {
    return new Promise(async (resolve, reject) => {
        ...
        if (!name)
            return reject('Please specify a package.');
...
function compileHandler(path = '.') {
    return new Promise((resolve, _reject) => {
        const child = child_process_1.exec('npm install; tsc --target ES2020 --module CommonJS --declaration --outDir ./lib --esModuleInterop --strict --removeComments --forceConsistentCasingInFilenames src/*.ts', {}, (error, stdout, stderr) => {
...
function _upload_dir(path, unlink = false) {
    return new Promise(async (resolve, reject) => {
        ...
        for (const filename of files) {
            ...
            if (stat.isDirectory()) {
                hasFailed || (hasFailed = !await _upload_dir(file, unlink)
                    .catch(_error => false));
            }
            else {
                hasFailed || (hasFailed = !await _upload_file(file, unlink)
                    .catch(_error => false));
            }
        }
        if (hasFailed)
...
async function cloudHandler(path = '.', { ignore = exports.DEFAULTS.CLOUD_HANDLER_IGNORE, keep = exports.DEFAULTS.CLOUD_HANDLER_KEEP, unlink = exports.DEFAULTS.CLOUD_HANDLER_UNLINK, unlink_by_default = false, }) {
    ...
    for (const filename of files) {
        ...
        if (!_ignore)
            success = (!!await _upload_dir(path_1.resolve(path, filename), _unlink).catch(() => { })) && success;
    }
    return success;
}
